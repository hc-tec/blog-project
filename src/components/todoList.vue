...
export default {
  ...
  methods: {
    ...
    initAddTag: function (name) {
      ...
      const id = (this.taskTags[this.taskTags.length - 1].id + 1) || 1
    ...
    getIdByParent (el) {
      const parent = el.currentTarget.parentNode
      return parent.getAttribute('id')
    },
    deleteTask (index, rows, el) {
      if (this.getUserInfo.power.isLogin) {
        const id = this.getIdByParent(el)
    ...
    initFinishTask: function () {
      const [id, index, rows, ..._] = arguments
      ajaxGet(
