...
export default {
  ...
  methods: {
    ...
    cmp (left, right) {
      ...
      //          right: h 标签名
      // 级层关系更小
      if (this.h_list.indexOf(left) > this.h_list.indexOf(right)) { return -1 }
      // 级层关系相等
      else if (this.h_list.indexOf(left) === this.h_list.indexOf(right)) { return 0 }
      // 级层关系更大
      else if (this.h_list.indexOf(left) < this.h_list.indexOf(right)) { return 1 }
    },
    formatKey (h_ele) {
      const h_ele_markup_name = h_ele.localName
      return `${h_ele_markup_name[1]}~~${h_ele.getAttribute('id')}`
    },
    toSameRow (index, h_ele_markup_name) {
      let flag = -1
      for (let i = index - 1; i >= 0; i--) {
        if (this.cmp(h_ele_markup_name, this.h_ele_list[i].localName) === -1) {
          flag = i
          break
        }
      }
    ...
    initMakeCatalog (alternate) {
      ...
      while (sum < this.h_ele_list.length) {
        ...
        const pres_row_list = row_list[row - 1] // 当前层级字典列表
        const first_formatKey = this.formatKey(this.h_ele_list[first]) // 第一级元素的 formatKey
        ...
        const chosen_list = this.h_ele_list.slice(first, last)
        const set = this.getSetOfH_ele(chosen_list) // 获取选中数组的集合，以判断其层级关系
        for (let i = first + 1; i < last; i++) {
          const pres_h_ele = this.h_ele_list[i] // 当前的 h 元素
          const pres_h_row = set.indexOf(pres_h_ele.localName) // 读取层级
          const formatKey = this.formatKey(pres_h_ele)
          try {
    ...
    unshowCatalog () {
      const catalog = document.getElementsByClassName('catalog-list')[0]
